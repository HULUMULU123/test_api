...
class CianCrawler:
    ...
    def _normalize_cian_url(self, href: str) -> str | None:
        ...
        if "cian.ru" not in parsed.netloc:
            return None
...
class CianCardParser:
    ...
    def _extract_address(self, soup: BeautifulSoup) -> str | None:
        ...
        json_ld = self._extract_json_ld(soup)
        address_data = json_ld.get("address") if json_ld else None
    ...
    def _parse_param_line(self, text: str) -> tuple[str, str] | None:
        ...
        for key, pattern in compact_patterns.items():
            match = re.search(pattern, text, flags=re.IGNORECASE)

            if match:
                return key, normalize_text(match.group(0))
