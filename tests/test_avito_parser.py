...
class HeadedBrowserDisplayTests(unittest.TestCase):
    ...
    def test_starts_xvfb_when_only_wayland_display_is_set(self) -> None:
        process = Mock()
        process.poll.return_value = None
        process.wait.return_value = None
        ...
        with (
            ...
            patch("app.avito_parser.shutil.which", return_value="/usr/bin/Xvfb"),
            patch("app.avito_parser._find_free_display_number", return_value=100),
            patch("app.avito_parser.subprocess.Popen", return_value=process),
            patch("app.avito_parser.time.sleep"),
        ):
            with headed_browser_display(headless=False):
